...
def listgenerator(lang, names, level=1, z=0):
    ...
    for x in range(0, len(names)):
        ...
        url = urllib.unquote("https://"+lang+".wikipedia.org/w/api.php?action=query&list=categorymembers&cmtitle=" + name + "&format=json&cmlimit=500&rawcontinue")
        try:
        ...
        try:
            ...
            nname1 = re.findall(r'"cmcontinue":"([^"]+)"}', str(m2))
            nname=nname1[0]
            normalno=0

        if normalno==1:
            elcount = len(data['query']['categorymembers'])

            for y in range(0, elcount):
                if data['query']['categorymembers'][y]['ns'] == nss :
                    artlist.append(data['query']['categorymembers'][y]['title'])
                if data['query']['categorymembers'][y]['ns'] == 14 :
                    catlist.append(data['query']['categorymembers'][y]['title'])

            if "query-continue" in data:
                 nname=data['query-continue']['categorymembers']['cmcontinue']
                 prodolzajem=1
        ...
        if prodolzajem==1:
            while prodolzajem==1:
                ...
                url2 = url + "&cmcontinue=" + urllib.unquote(nname).encode('utf-8')
                try:
